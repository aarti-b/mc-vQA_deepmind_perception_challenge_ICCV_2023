...
# Set up a CSV writer
csv_file = open('training_log_perceiver_vanilla.csv', 'w', newline='')
...
for epoch in range(epochs):
    ...
    for dataset in train_dataloader:
        ...
        if running_loss>loss_cumulative:
            model_save_path = f'model_vqa_perceiver_vanilla_optimised_epoch_{epoch}_{init_batch}_{round(loss_cumulative.item(),4)}.pth'
        ...
        if init_batch%10==0:
            model_save_path = f'model_vqa_perceiver_vanilla_epoch_{epoch}_{init_batch}.pth'
            torch.save(model.state_dict(), model_save_path)
            print(f'Model saved to {model_save_path} for epoch {epoch}')
    ...
    model_save_path = f'model_vqa_perceiver_epoch_{epoch}.pth'
    torch.save(model.state_dict(), model_save_path)
    print(f'Model saved to {model_save_path} for epoch {epoch}')

model_save_path = 'final_model_VQA_perceiver.pth'
torch.save(model.state_dict(), model_save_path)
